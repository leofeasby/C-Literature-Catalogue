// Description: File handling for the database.
// Author: Leo Feasby
// Date: 11/05/2024
      ...
      switch (mode) 
      {
        ...
        case ReadingMode::Theses:
        {
          if (tokens.size() >= 5) 
          {
            ...
            std::string supervisor = tokens[3];
            std::string university = tokens[4];
            Thesis thesis(id, title, author, supervisor, university);
            theses.push_back(thesis);
            thesis_map[thesis.id] = &theses.back(); 
            literature_title_map[{thesis.id, 'T'}] = thesis.title;
          }
        }
        break;
        case ReadingMode::Journals:
        {
          try 
          {
            ...
            journal_map[journal.id] = &journals.back(); 
            literature_title_map[{journal.id, 'J'}] = journal.title; 
          } 
